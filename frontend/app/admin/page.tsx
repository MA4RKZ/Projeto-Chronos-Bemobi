...
export default function AdminPage() {
  ...
  // === Links diretos para abrir Notion e Google Drive ===
  const notionUrl = useMemo(() => {
    const db = conns?.notion?.database_id
    if (!db) return null
    return `https://www.notion.so/${String(db).replace(/-/g, '')}`
  }, [conns])

  const gdriveUrl = useMemo(() => {
    const fid = conns?.gdrive?.folder_id
    if (!fid) return null
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            ...
            <div className="flex items-center gap-2">
              {conns?.notion?.enabled && notionUrl && (
                <a
                  href={notionUrl}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="px-3 py-2 bg-white border border-gray-300 rounded-xl text-sm hover:bg-gray-50"
                >
                  Abrir Notion
                </a>
              )}
              {conns?.gdrive?.enabled && gdriveUrl && (
                <a
                  href={gdriveUrl}
                  target="_blank"
                  rel="noopener noreferrer"
        ...
        <section className="chronos-card p-8">
          <div className="flex items-center justify-between mb-6">
            ...
            <div className={`flex items-center gap-2 px-3 py-1 rounded-full text-sm ${
              anyEnabled ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'
            }`}>
              <div className={`w-2 h-2 rounded-full ${anyEnabled ? 'bg-green-500' : 'bg-red-500'}`} />
              {anyEnabled ? 'Conectado' : 'Desconectado'}
            </div>
          </div>
          
          <div className="space-y-4">
            {Object.entries(conns).map(([name, cfg]: any) => {
              const isNotion = name === 'notion'
              const isGDrive = name === 'gdrive'
              const rightLink =
                isNotion && cfg.enabled && conns?.notion?.database_id ? (
                  <a
                    href={`https://www.notion.so/${String(conns.notion.database_id).replace(/-/g, '')}`}
                    target="_blank"
                    rel="noopener noreferrer"
                ...
                ) : isGDrive && cfg.enabled && conns?.gdrive?.folder_id ? (
                  <a
                    href={`https://drive.google.com/drive/folders/${conns.gdrive.folder_id}`}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="text-blue-600 hover:underline text-sm mr-4"
                  >
                    ...
                    <label className="inline-flex items-center gap-3 cursor-pointer">
                      ...
                      <div className="relative">
                        ...
                        <div className={`w-12 h-6 rounded-full transition-colors ${cfg.enabled ? 'bg-green-500' : 'bg-gray-300'}`}>
                          <div className={`w-5 h-5 bg-white rounded-full shadow-md transform transition-transform ${cfg.enabled ? 'translate-x-6' : 'translate-x-0.5'} mt-0.5`} />
                        </div>
                      </div>
                    </label>
                  </div>
          ...
          <div className="flex items-start gap-4">
            ...
            <div>
              ...
              <p className="text-orange-700 text-sm leading-relaxed">
                Edite o arquivo <code className="bg-orange-200 px-2 py-1 rounded text-xs">backend/app/data/connectors.json</code> para incluir integrações com Notion, Google Drive ou Microsoft 365.
                Após a configuração, ative o conector desejado e clique em <b>Sincronizar</b> para importar os dados (ou <b>Reindexar</b> para reconstruir do zero).
              </p>
            </div>
          </div>
        </div>
